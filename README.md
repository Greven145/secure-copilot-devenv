...
- **devcontainer.json** — `cap-drop=ALL` + `cap-add=NET_BIND_SERVICE`, `no-new-privileges`, `--memory=4g`/`--cpus=2` resource limits, Docker-in-Docker (isolated daemon, no host socket mount), `NODE_TLS_REJECT_UNAUTHORIZED=1`, named volumes for credentials/caches so they survive rebuilds without living in the image.
